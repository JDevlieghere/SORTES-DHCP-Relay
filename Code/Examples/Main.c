...
********************************************************************/
/*************ML changes: to adapt to sdcc compiler*****************/
#include <string.h>
#include <stdlib.h>

#define __18F97J60 //ML
#include <p18f97j60.h> //ML


//ML #include <p18cxxx.h>
//ML#include "GenericTypeDefs.h"
#include "Include/GenericTypeDefs.h" //ML
//ML #include "TCPIP Stack/ETH97J60.h"
#include "Include/TCPIP_Stack/ETH97J60.h" //ML
...
#define TOTAL_MEMORY_SIZE	INTERNAL_FLASH_SIZE	// Add more memory here
// if you are using an EMB and would like the bootloader to access it.  
// NOTE: EMB support is not currently implemented.
...
// Pseudo Functions: Intended for compile time use only (not as efficient as 
// casting to a pointer of a 16 bit type and then taking the proper byte)
#define LOW(a) 				(a & 0xFF)
...
static void TFTPBootloadProcess(ETH_HEADER* ETHHeaderPtr, 
      IP_HEADER* IPHeaderPtr, UDP_HEADER* UDPHeaderPtr, BYTE* AppDataPtr, 
      WORD AppDataLen);
static CHAR DecodeHex(BYTE *vHexData, WORD *wHexDataLen, BYTE *vMemoryData, 
      DWORD *dwAddress, BOOL bInitialize);
...
static BOOL RawWriteFlashBlock(DWORD Address, BYTE *BlockData, 
      BYTE *StatusData);
...
void main(void)
{
	...
	// variables get allocated on the stack before we can initialize the 
        //stack pointer
	...
	static BOOL			bLEDOn;
	static BYTE 		i;
	static BYTE			vExitRCONState;	// This byte must not 
	...
	if(RCONbits.NOT_POR)
	{
		// Change the PC to the application start vector, if an 
                // application has been bootloaded
		if(dwJumpInstructions[0] != 0xFFFFFFFF)
		{
			...
			PCLATU = dwvNewPC.v[2];
			PCLATH = dwvNewPC.v[1];
			PCL    = dwvNewPC.v[0]+1-1;    //+1-1 is required to 
			//prevent C18 from using MOVFF instruction, which 
			//cannot move to the PCL register
		}
	}
