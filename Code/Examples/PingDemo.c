...
//ML					#if defined(STACK_USE_DNS)
//ML						ICMPSendPingToHostROM((ROM BYTE*)HOST_TO_PING);
//ML					#else
...
//ML					#endif
		...
		case SM_GET_ICMP_RESPONSE:
			...
			else if(ret == -1)
			{
				// Request timed out
				#if defined(USE_LCD)
//ML				memcpypgm2ram((void*)&LCDText[16], (ROM void *)"Ping timed out", 15);
			  memcpy((void*)&LCDText[16], (void *)"Ping timed out", 15); //ML
				LCDUpdate();
				#endif
				PingState = SM_HOME;
			}
			else if(ret == -3)
			{
				...
				#if defined(USE_LCD)
//ML				memcpypgm2ram((void*)&LCDText[16], (ROM void *)"Can't resolve IP", 16);
				memcpy((void*)&LCDText[16], ( void *)"Can't resolve IP", 16);
				LCDUpdate();
				#endif
				PingState = SM_HOME;
			}
			else
			{
				...
				#if defined(USE_LCD)
//ML				memcpypgm2ram((void*)&LCDText[16], (ROM void *)"Reply: ", 7);
				memcpy((void*)&LCDText[16], ( void *)"Reply: ", 7);
//ML				uitoa((WORD)TickConvertToMilliseconds((DWORD)ret), &LCDText[16+7]);
				uitoa((WORD)TickConvertToMilliseconds((DWORD)ret), &LCDText[16+7],10);
//ML				strcatpgm2ram((char*)&LCDText[16+7], (ROM char*)"ms");
				strcat((char*)&LCDText[16+7], ( char*)"ms");
				LCDUpdate();
				#endif
				PingState = SM_HOME;
			}
