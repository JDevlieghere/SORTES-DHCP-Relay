...
/*****************************************************************************
  ...
  ***************************************************************************/
//MLBYTE hexatob(WORD_VAL AsciiChars)
BYTE hexatob(WORD AsciiCharsarg)
{
        WORD_VAL AsciiChars ; //ML
        AsciiChars.v[0] = (BYTE)AsciiCharsarg&0xFF; //ML
        AsciiChars.v[1] = (BYTE)(AsciiCharsarg>>8)&0xFF; //ML
 
	// Convert lowercase to uppercase
	if(AsciiChars.v[1] > (int)'F')
		AsciiChars.v[1] -= (int)'a'-(int)'A';
	if(AsciiChars.v[0] > (int)'F')
		AsciiChars.v[0] -= (int)'a'-(int)'A';

	// Convert 0-9, A-F to 0x0-0xF
	if(AsciiChars.v[1] > (int)'9')
		AsciiChars.v[1] -= (int)'A' - 10;
	else
		AsciiChars.v[1] -= '0';

	if(AsciiChars.v[0] > (int)'9')
		AsciiChars.v[0] -= (int)'A' - 10;
	else
		AsciiChars.v[0] -= (int)'0';

	// Concatenate
	//ML	return (AsciiChars.v[1]<<4) |  AsciiChars.v[0];
        return (WORD)((AsciiChars.v[1]<<4) |  AsciiChars.v[0]);
...
DWORD leftRotateDWORD(DWORD val, BYTE bits)
{
	...
	{
		_asm
		  movlb _toRotate  //_ added by ML
		bcf STATUS,0,0
		btfsc _toRotate+3,7,1  //_ added by ML
		bsf STATUS,0,0
		rlcf _toRotate+0,1,1  //_ added by ML
		rlcf _toRotate+1,1,1  //_ added by ML
		rlcf _toRotate+2,1,1  //_ added by ML
		rlcf _toRotate+3,1,1  //_ added by ML
	        _endasm;
	}
	#endif
