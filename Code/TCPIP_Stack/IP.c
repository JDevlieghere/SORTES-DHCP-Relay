...
#define MAX_OPTIONS_LEN     (40u)            // As per RFC 791.
    ...
    if(header.FragmentInfo & 0xFF1F) return FALSE;
    ...
    if ( optionsLen > MAX_OPTIONS_LEN ) return FALSE;

    if ( optionsLen > 0u ) MACGetArray(options, optionsLen);
                 ...
                 WORD len)
{
    ...
    header.HeaderChecksum   = 0;
//ML	header.SourceAddress 	= AppConfig.MyIPAddr;
