 ...
 *                               2. UDPProcess was incorrectly calculating 
 *                                  length.
...
static UDP_SOCKET  LastPutSocket = INVALID_UDP_SOCKET;  // Indicates the 
                        //last socket to which data was written
static WORD wPutOffset;	// Offset from beginning of payload where data is to be
                        // written.
static WORD wGetOffset;	// Offset from beginning of payload from where data is 
                        // to be read.

// Stores various flags for the UDP module
static struct
{
    unsigned char bFirstRead : 1;      // No data has been read from this 
                                       // segment yet
    unsigned char bWasDiscarded : 1;   // The data in this segment has been 
                                       // discarded
} Flags;
...
{
    UDP_SOCKET s;
//DisplayString(8,"udpinit"); //ML debug
...
/*****************************************************************************
  ...
  Description:
	Performs state management and housekeeping for UDP.  This is an 
	internal function meant to be called by StackTask() (not a user API).
  ...
  Remarks:
	UDPTask() is called once per StackTask() iteration to ensure that  
	calls to UDPIsPutReady() always update the Ethernet Write pointer 
	location between StackTask() iterations.
  ***************************************************************************/
void UDPTask(void)
{
...
/*****************************************************************************
  ...
  Parameters:
	localPort - UDP port number to listen on.  If 0, stack will  
		dynamically assign a unique port number to use.
  ...
  Return Values:
  	Success -
		A UDP socket handle that can be used for subsequent UDP API 
                calls.
	Failure -
		INVALID_UDP_SOCKET.  This function fails when no more UDP 
		socket handles are available.  Increase MAX_UDP_SOCKETS to make
		 more sockets available.
  	
  Remarks:
	When finished using the UDP socket handle, call the UDPClose()  
	function to free the socket and delete the handle.
  ***************************************************************************/
UDP_SOCKET UDPOpen(UDP_PORT this_localPort,
                   NODE_INFO *this_remoteNode,
                   UDP_PORT this_remotePort)
{
    UDP_SOCKET s;
    UDP_SOCKET_INFO *p;
    ...
    p = UDPSocketInfo; //pointer to the table of UDP sockets
    for ( s = 0; s < MAX_UDP_SOCKETS; s++ )
    {
        if(p->localPort == INVALID_UDP_PORT)
        {
	    p->localPort = this_localPort;	

	    if(this_localPort == 0x0000u) //select any free port
	    {
            ...
            if(this_remoteNode)
            {
                memcpy((void*)&(p->remoteNode), (void*)this_remoteNode, 
                        sizeof(p->remoteNode));
            }
            else
	    {
...
/*****************************************************************************
  ...
  Description:
	Closes a UDP socket and frees the handle.  Call this function to 
	release a socket and return it to the pool for use by future 
        communications.
...
/*****************************************************************************
  ...
  Description:
	This function writes an array of bytes to the currently active UDP 
	socket, while incrementing the buffer length.  UDPIsPutReady should be 
	used before calling this function to specify the currently active 
        socket.
...
WORD UDPPutArray(BYTE *cData, WORD wDataLen)
{
    ...
    if(wTemp < wDataLen) wDataLen = wTemp;

    wPutOffset += wDataLen;
    if(wPutOffset > UDPTxCount) UDPTxCount = wPutOffset;
...
/*****************************************************************************
  Function:
	BYTE* UDPPutROMString(BYTE *strData)
...
/*****************************************************************************
  ...
  Description:
	...
	for transmission over the network interface.  Since UDP is a
	frame-based protocol, this function must be called before returning 
        to the main stack loop whenever any data is written.

  Precondition:
	UDPIsPutReady() was previously called to specify the current socket, 
        and data has been written to the socket using the UDPPut family of 
        functions.
...
void UDPFlush(void)
{
	...
	pseudoHeader.SourceAddress.Val = AppConfig.MyIPAddr.Val;
	pseudoHeader.DestAddress.Val = p->remoteNode.IPAddr.Val;
	pseudoHeader.Zero = 0x0;
	pseudoHeader.Protocol = IP_PROT_UDP;
	pseudoHeader.Length = wUDPLength;
	SwapPseudoHeader(pseudoHeader);
	h.Checksum = ~CalcIPChecksum((BYTE*)&pseudoHeader, 
                         sizeof(pseudoHeader));
    }
    #endif
    ...
    #if defined(UDP_USE_TX_CHECKSUM)
    {
	wReadPtrSave = MACSetReadPtr(BASE_TX_ADDR + sizeof(ETHER_HEADER) + 
                            sizeof(IP_HEADER));
	wChecksum = CalcIPBufferChecksum(wUDPLength);
	MACSetReadPtr(wReadPtrSave);
	MACSetWritePtr(BASE_TX_ADDR + sizeof(ETHER_HEADER) + sizeof(IP_HEADER)
              + 6);    // 6 is the offset to the Checksum field in UDP_HEADER
    ...
    // Transmit the packet
    MACFlush();
//DisplayString(0,"MACFlush'd"); //ML debug
...
{
    activeUDPSocket = s;
    if(SocketWithRxData != s) return 0;
...
/*****************************************************************************
  ...
  Description:
	...
	while decrementing the remaining buffer length.  UDPIsGetReady should 
        be used before calling this function to specify the currently active 
        socket.
...
WORD UDPGetArray(BYTE *cData, WORD wDataLen)
{
    ...
    if(wBytesAvailable < wDataLen) wDataLen = wBytesAvailable;
...
BOOL UDPProcess(NODE_INFO *remoteNode, IP_ADDR *localIP, WORD len)
{
	...
	pseudoHeader.SourceAddress.Val	= remoteNode->IPAddr.Val;
	...
	// Now calculate UDP packet checksum in NIC RAM: should match 
        // pseudoHeader
	IPSetRxBuffer(0);
...
/*****************************************************************************
  Function:
	static UDP_SOCKET FindMatchingSocket(UDP_HEADER *h, NODE_INFO 
                         *remoteNode, IP_ADDR *localIP)
                                     ...
                                     IP_ADDR *localIP)
{
    ...
    if ( partialMatch != INVALID_UDP_SOCKET )
    {
        ...
        memcpy((void*)&(p->remoteNode),
                (void*)remoteNode, sizeof(p->remoteNode) );
